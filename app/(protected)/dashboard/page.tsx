          ...
          {data.recentPets.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {data.recentPets.map((pet) => (
                <Card key={pet.id} className="cursor-pointer hover:bg-accent transition-colors" onClick={() => router.push(`/pets/${pet.id}`)}>
              ...
              {data.recentAppointments.length === 0 ? (
                ...
                <div className="space-y-4">
                  {data.recentAppointments.map((apt) => (
                    <div key={apt.id} className="flex justify-between items-center p-4 bg-accent rounded-lg">
                      <div>
                        <p className="font-semibold">{capitalizeWords(apt.type)}</p>
                        <p className="text-sm text-muted-foreground">{format(parseISO(apt.date), 'PPP')}</p>
                      </div>
                      <Button variant="outline" size="sm" onClick={() => router.push(`/pets/${apt.petId}/appointments`)}>
                        View
                      </Button>
                    </div>
                  ))}
              ...
              {data.recentMedications.length === 0 ? (
                ...
                <div className="space-y-4">
                  {data.recentMedications.map((med) => (
                    <div key={med.id} className="flex justify-between items-center p-4 bg-accent rounded-lg">
                      <div>
                        <p className="font-semibold">{med.name}</p>
                        <p className="text-sm text-muted-foreground">Due: {format(parseISO(med.nextDueDate), 'PPP')}</p>
                      </div>
                      <Button variant="outline" size="sm" onClick={() => router.push(`/pets/${med.petId}/medications`)}>
                        View
                      </Button>
                    </div>
                  ))}
