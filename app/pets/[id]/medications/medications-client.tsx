...
import { Calendar, Pill, Weight, Edit, Cake, PawPrint, Clock, Timer } from 'lucide-react';
...
export function PetOverviewClient({ petId }: PetOverviewClientProps) {
  ...
  const [upcomingMedicationReminders, setUpcomingMedicationReminders] = useState<MedicalRecord[]>([]); // New state
  ...
  useEffect(() => {
    ...
    const fetchPetData = async () => {
      try {
        ...
        // Upcoming Medication Reminders Query
        const upcomingMedicationRemindersQuery = query(
          collection(db, 'medications'),
          where('petId', '==', petId),
        ...
        const unsubUpcomingMedicationReminders = onSnapshot(upcomingMedicationRemindersQuery, (snapshot) => {
          const reminders: MedicalRecord[] = [];
          snapshot.forEach((doc) => {
            reminders.push({ id: doc.id, ...doc.data() } as MedicalRecord);
          });
          setUpcomingMedicationReminders(reminders);
        });
        ...
        return () => {
          unsubAppointments();
          unsubMedications();
          unsubUpcomingMedicationReminders();
          unsubWeights();
        };
      ...
      <Tabs defaultValue="appointments" className="space-y-4">
        ...
        <TabsContent value="medications" className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => router.push(`/pets/${petId}/medications`)}>
            ...
            {recentMedications.length === 0 ? (
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">Add a new medication</p>
                  <Button onClick={() => router.push(`/pets/${petId}/medications`)}>
                    Add Medication
                  </Button>
                </CardContent>
              ...
              recentMedications.map((medication) => (
                <Card key={medication.id} className="cursor-pointer hover:bg-accent/50" onClick={() => router.push(`/pets/${petId}/medications`)}>
                  <CardContent className="flex justify-between items-center p-6">
                    <div className="space-y-1">
        ...
        <TabsContent value="weight" className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => router.push(`/pets/${petId}/weight`)}>
              View All Weights
            </Button>
          </div>
          <div className="grid gap-4">
